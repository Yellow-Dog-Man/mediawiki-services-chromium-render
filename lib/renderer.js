...
class Renderer {
     ...
     * @param {Object} data Extra meta-data from the @see {QueueItem}
     * @return {<BBPromise<Object>>} Promise which resolves with PDF buffer
     */
    articleToPdf(url, format, headers, data) {
        let page;
    ...
    processUrl(pageUrlStr, data) {
        const url = new URL(pageUrlStr);
        if (data.theme) {
            url.searchParams.set('pdftheme', data.theme);
        }
        return url.toString();
    }
